...
def test_mule_piecewise_age_logic():
    ...
    trades_hist = pd.DataFrame({
        ...
        "ts": pd.date_range("2025-01-01", periods=50, freq="h"),  # use 'h' (lowercase) per pandas warning
        "buyer_id":["X"]*50,
        "seller_id":["Y"]*50,
        "item_id":["I1"]*50,
        "qty":[1]*50,
    ...
    # target trade: price 50 (==50% of ref ~100) from seller S to buyer B
    ...
    # Case 2: 28-day buyer on a HIGH-VALUE ITEM (item p95 ≫ median), still cheap (≤ 50%)
    high_trade = new_trade.copy()
    high_trade.loc[0,"trade_id"] = "T2"
    high_trade.loc[0,"price"] = 50     # keep cheapness condition
